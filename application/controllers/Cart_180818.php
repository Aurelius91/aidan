...
class Cart extends CI_Controller
{
    ...
    public function payment()
    {
        ...
        if ($shipping->country_id == 233)
        {
            ...
            {
                $shipping->price_fix = ceil($shipping->price / $currency->currency_exchange) * $weight;
            }
            else
            {
                $shipping->price_fix = ceil($shipping->price / $currency->currency_exchange);
            }
        ...
        else
        {
            ...
            $shipping->price_fix = ceil($price / $currency->currency_exchange) * $weight;
            $shipping->price_fix_display = $currency->name . ' ' . number_format($shipping->price_fix, 0, ',', '.');
        }

        $discount_display = '';
        $discount = $discount / $currency->currency_exchange;
    ...
    public function ajax_generate_order_number()
    {
        ...
        try
        {
            ...
            {
                $price = $shipping->price * $selected_currency->currency_real;
    ...
    public function ajax_get_voucher($voucher_number = '')
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $voucher_code->value = ceil($voucher_code->value / $currency->currency_exchange);

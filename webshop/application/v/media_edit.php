	...
	<script type="text/javascript">
		...
		function reset() {
			$('#media-name').val("<?= $media->name; ?>");
			$('#media-subtitle').val("<?= $media->subtitle; ?>");
			$('#media-date').val("<?= $media->date_display; ?>");

			$('#media-product').val("");
			$('#media-product-container').dropdown('set selected', [<? foreach ($media->arr_media_product as $media_product): ?>'<?= $media_product->product_id; ?>',<? endforeach; ?>]);

			$('#main-image').val("");
			$('#preview').data('image_id', 0);

			var image = '<img class="preview" style="width: 100%;" src="<?= base_url(); ?>images/website/<?= $media->image_name; ?>">';
			$('#preview').append(image);
		}
		...
		function submit() {
			...
			$.ajax({
				...
				type : 'POST',
				url : '<?= base_url() ?>media/ajax_edit/<?= $media->id; ?>/',
				xhr: function() {
					var percentage = 0;
					var xhr = new window.XMLHttpRequest();
		...
		<div class="ui stackable one column centered grid">
			<div class="column">
				<div class="ui attached message setting-header">
					<div class="header">Edit Media</div>
				</div>
				<div class="form-content">
					<div class="ui form">
						...
						<div class="field">
							<label>Content</label>
							<textarea id="media-description" placeholder="Content.."><?= $media->description; ?></textarea>
						</div>

	...
	<script type="text/javascript">
		...
		function deletecollection() {
			var collectionId = $('.delete-collection-button').attr('data-collection-id');
			var collectionUpdated = $('.delete-collection-button').attr('data-collection-updated');
			...
			$.ajax({
				...
				type : 'POST',
				url : '<?= base_url() ?>collection/ajax_delete/'+ collectionId +'/',
				xhr: function() {
					var percentage = 0;
		...
		function filter(page) {
			...
			window.location.href = '<?= base_url(); ?>collection/view/'+ page +'/'+ filterQuery +'/'+ searchQuery +'/';
		}
		...
		function collectionClick() {
			...
			$('.open-modal-warning-delete').click(function() {
				...
				var collectionName = $(this).attr('data-collection-name');
				var collectionUpdated = $(this).attr('data-collection-updated');

				$('.delete-collection-title').html('Delete collection ' + collectionName);
				$('.delete-collection-button').attr('data-collection-id', collectionId);
				$('.delete-collection-button').attr('data-collection-updated', collectionUpdated);

				$('.ui.basic.modal.modal-warning-delete').modal('show');
			});
		}

		function collectionKeypress() {
	...
	<div class="ui basic modal modal-warning-delete">
		<div class="ui icon header">
			...
			<span class="delete-collection-title">Delete collection</span>
		</div>
		<div class="content text-center">
			<p>You're about to delete this collection. You will not be able to undo this action. Are you sure?</p>
		</div>
		<div class="actions">
			...
			<div class="ui green ok inverted button delete-collection-button" onclick="deletecollection();">
	...
	<div class="main-content">
		<div class="ui top attached menu table-menu">
			...
			<div class="right menu">
				<? if (isset($acl['collection']) && $acl['collection']->add != ''): ?>
					<a class="item item-add-button" href="<?= base_url(); ?>collection/add/">
						<i class="add circle icon"></i> Add Collection
					</a>
				<? endif; ?>
				<div class="item">
							...
							<tr>
								<td class="td-icon">
									<? if (isset($acl['collection']) && $acl['collection']->edit > 0): ?>
										<a href="<?= base_url(); ?>collection/edit/<?= $collection->id; ?>/">
											<span class="table-icon" data-content="Edit collection">
												<i class="edit icon"></i>
											</span>
										</a>
									<? endif; ?>
									...
									<? if (isset($acl['collection']) && $acl['collection']->delete > 0): ?>
										<a class="open-modal-warning-delete" data-collection-id="<?= $collection->id; ?>" data-collection-name="<?= $collection->name; ?>" data-collection-updated="<?= $collection->updated; ?>">
											<span class="table-icon" data-content="Delete collection">
									...
									<? if (isset($acl['room']) && $acl['room']->list > 0): ?>
										<a href="<?= base_url(); ?>collection_room/view/<?= $collection->id; ?>/">

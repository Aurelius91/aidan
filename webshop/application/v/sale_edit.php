	...
	<script type="text/javascript">
		...
		function calculateTotal(subtotal) {
			...
			var subtotalDiscountTax = (parseInt(tax) / 100) * (parseInt(subtotal) - parseInt(subtotalDiscount));
		...
		function reset() {
			$('#sale-number').val("<?= $sale->number; ?>");
			$('#sale-date').val("<?= $sale->date_display; ?>");
			$('#sale-discount').val('<?= $sale->discount_display; ?>');
			$('#sale-shipping').val('<?= $sale->shipping_display; ?>');
			$('#sale-tax').val('<?= $sale->tax_display; ?>');

			$('#sale-deadline').val("<?= $sale->deadline_display; ?>");

			$('#sale-location').val("<?= $sale->location_id; ?>");
			$('#sale-location-selection').dropdown('set selected', "<?= $sale->location_id; ?>");

			$('#sale-customer').val("<?= $sale->customer_id; ?>");
			$('#sale-customer-selection').dropdown('set selected', "<?= $sale->customer_id; ?>");

			<? if ($setting->system_mac != 'webshop_2_aidan'): ?>
				$('#sale-method').val("<?= $sale->type; ?>");
				$('#sale-method-selection').dropdown('set selected', "<?= $sale->type; ?>");
			<? else: ?>
				$('#sale-method').val("<?= $sale->status; ?>");
				$('#sale-method-selection').dropdown('set selected', "<?= $sale->status; ?>");
			<? endif; ?>

			$('#sale-statement').val("<?= $sale->statement_id; ?>");
			$('#sale-statement-selection').dropdown('set selected', "<?= $sale->statement_id; ?>");

			<? foreach ($sale->arr_sale_item as $key => $sale_item): ?>
				$('#sale-product-selection-<?= $key + 1; ?>').dropdown('set selected', "<?= $sale_item->product_id; ?>");
				$('#sale-product-price-<?= $key + 1; ?>').val("<?= $sale_item->price_display; ?>");
				$('#sale-product-quantity-<?= $key + 1; ?>').val("<?= $sale_item->quantity_display; ?>");
			<? endforeach; ?>
		...
		function submit(draft) {
			...
			$.ajax({
				...
				type : 'POST',
				url : '<?= base_url() ?>sale/ajax_edit/<?= $sale->id; ?>',
				xhr: function() {
					var percentage = 0;
	...
	<div class="ui modal shipping-address-modal">
		<i class="close icon"></i>
		<div class="header">Shipping Address</div>
		<div class="user-access-list content">
			<div style="padding: 1.25rem 1.5rem;">
				<div>To: <?= $sale->shipping_name; ?></div>
				<div>Address: <?= $sale->shipping_address; ?></div>
				<div><?= $sale->shipping_district; ?>, <?= $sale->shipping_city; ?></div>
				<div><?= $sale->shipping_province; ?>, <?= $sale->shipping_zip_code; ?>, , <?= $sale->shipping_country; ?></div>
				<div>Phone: <?= $sale->shipping_phone; ?></div>
				<div>Email: <?= $sale->shipping_email; ?></div>
				<div style="padding-top: 15px;">Courier: <?= $sale->shipping_courier; ?></div>
				<div>ETD: <?= $sale->shipping_etd; ?></div>
				<div>Price: <?= $sale->shipping_display; ?></div>

				<div>Message: <?= $sale->message; ?></div>
			</div>
		</div>
	</div>
		...
		<div class="ui stackable one column centered grid">
			<div class="column">
				<div class="ui attached message setting-header">
					<div class="header"><? if ($sale->draft > 0): ?>[DRAFT]<? endif; ?> Edit Sale</div>
				</div>
						...
						<div class="field">
							<div class="four fields">
								<div class="field">
									...
									<input id="sale-number" class="form-input" placeholder="AUTO.." type="text" <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>>
								</div>
								<div class="field">
									<label>Date</label>
									<input id="sale-date" class="form-input" placeholder="Date.." type="text" <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>>
								</div>
									...
									<div class="field">
										...
										<div id="sale-method-selection" class="ui search selection dropdown form-input">
											...
											<div class="menu">
												...
												<div class="item" data-value="capture">Credit Card Payment</div>
												<div class="item" data-value="settlement">Bank Transfer Completed</div>
												<div class="item" data-value="shipping">Shipping</div>
								...
								<? else: ?>
									<div class="field">
										...
										<div id="sale-method-selection" class="ui search disabled selection dropdown form-input">
											<input id="sale-method" type="hidden" class="data-important">
											<i class="dropdown icon"></i>
											<div class="default text">-- Select Status --</div>
											<div class="menu">
								...
								<div class="field">
									<label>Deadline (<span id="deadline-term"><?= $sale->term; ?></span> Days)</label>
							...
							<div class="three fields">
								<div class="field">
									...
									<div id="sale-location-selection" class="ui search selection <? if ($account->location_id > 0): ?>disabled<? endif; ?> <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?> dropdown form-input">
								...
								<div class="field">
									...
									<div id="sale-customer-selection" class="ui search selection dropdown <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?> form-input">
								...
								<div class="field">
									<label>Account</label>
									<div id="sale-statement-selection" class="ui search selection dropdown <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?> form-input">
							...
							<div class="field">
								<button class="ui button shipping-address-button">Show Shipping Address</button>
							</div>
							...
							<div class="field">
								<table class="ui striped selectable celled table" style="border: 1px solid rgba(34, 36, 38, 0.15); border-radius: 0;">
									...
									<tbody id="sale-item-list">
										<? foreach ($sale->arr_sale_item as $key => $sale_item): ?>
											<tr id="sale-item-list-<?= $key + 1; ?>" class="sale-product-list" data-number="<?= $key + 1; ?>">
												<td class="td-icon">
												...
												<td>
													<? if ($setting->system_mac == 'webshop_2_aidan'): ?>
														<? if ($sale_item->voucher_id > 0): ?>
															<div><?= $sale_item->voucher_name; ?></div>

															<? foreach ($arr_voucher_code_lookup[$sale_item->id] as $voucher_code): ?>
																<div style="margin-top: 10px;">
																	<div>Code: <?= $voucher_code->number; ?></div>

																	<? if ($voucher_code->voucher_type == 'Virtual'): ?>
																		<div>Recipient Name: <?= $voucher_code->recipient_name; ?></div>
																		<div>Recipient Email: <?= $voucher_code->recipient_email; ?></div>
																	<? endif; ?>

																	<div>Message: <?= $voucher_code->message; ?></div>
																</div>
															<? endforeach; ?>
														<? else: ?>
															<div><?= $sale_item->product_number; ?> = <?= $sale_item->product_name; ?></div>
														<? endif; ?>
													<? else: ?>
														<div id="sale-product-selection-<?= $key + 1; ?>" class="ui search selection dropdown form-input <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>">
															<input id="sale-product-<?= $key + 1; ?>" class="sale-product-list-selection" data-number="<?= $key + 1; ?>" type="hidden" class="data-important">
															<i class="dropdown icon"></i>
												...
												<td class="td-price-quantity" style="text-align: right;">
													<input id="sale-product-price-<?= $key + 1; ?>" type="text" class="sale-item-price" data-number="<?= $key + 1; ?>" placeholder="Price.." style="text-align: right;" <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>>
												</td>
												<td class="td-price-quantity" style="text-align: right;">
													<input id="sale-product-quantity-<?= $key + 1; ?>" type="text" class="sale-item-quantity" data-number="<?= $key + 1; ?>" placeholder="Quantity.." style="text-align: right;" <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>>
												</td>
												<td class="td-price-quantity" style="text-align: right;">
													<span id="sale-product-price-total-<?= $key + 1; ?>" data-number="<?= $key + 1; ?>" data-total="0">Rp 0</span>
												</td>
											</tr>
										<? endforeach; ?>
									...
									<tfoot>
										...
										<tr>
											...
											<td style="text-align: right;">
												<input id="sale-discount" type="text" data-number="1" placeholder="Discount.." style="text-align: right;" <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>>
											</td>
											<td id="sale-discount-display" style="text-align: right;">Rp 0</td>
										</tr>
										<tr <? if ($setting->system_mac == 'webshop_2_aidan'): ?>style="display: none;"<? endif; ?>>
											<td colspan="3" style="text-align: right;">PPN (%)</td>
											<td style="text-align: right;">
												<input id="sale-tax" type="text" data-number="1" placeholder="PPN.." style="text-align: right;" <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>>
											</td>
										...
										<tr>
											...
											<td style="text-align: right;">
												<input id="sale-shipping" type="text" data-number="1" placeholder="Shipping.." style="text-align: right;" <? if ($setting->system_mac == 'webshop_2_aidan'): ?>disabled<? endif; ?>>
											</td>

	...
	<script type="text/javascript">
		...
		function back() {
			window.location.href = '<?= base_url(); ?>alterego/view/1/';
		}
		...
		function reset() {
			$('#alterego-name').val("<?= $alterego->name; ?>");
			$('#alterego-description').val("<?= $alterego->description; ?>");
			...
			var image = '<img class="preview" style="width: 100%;" src="<?= base_url(); ?>images/website/<?= $alterego->image_name; ?>">';
			$('#preview').append(image);
		}

		function submit() {
			var alteregoName = $('#alterego-name').val();
			var alteregoDescription = $('#alterego-description').val();
			...
			$.ajax({
				data :{
					name: alteregoName,
					description: alteregoDescription,
					image_id: imageId,
					"<?= $csrf['name'] ?>": "<?= $csrf['hash'] ?>"
				},
				dataType: 'JSON',
				...
				type : 'POST',
				url : '<?= base_url() ?>alterego/ajax_edit/<?= $alterego->id; ?>/',
				xhr: function() {
					var percentage = 0;
					var xhr = new window.XMLHttpRequest();
		...
		<div class="ui stackable one column centered grid">
			<div class="column">
				<div class="ui attached message setting-header">
					<div class="header">Edit Alter Ego</div>
				</div>
				<div class="form-content">
					<div class="ui form">
						<h4 class="ui dividing header">Alter Ego - Details</h4>
						<div class="two fields">
							<div class="field">
								<label>Name <span class="color-red">*</span></label>
								<input id="alterego-name" class="form-input data-important" placeholder="Name.." type="text">
							</div>
							<div class="field">
								<label>Description</label>
								<input id="alterego-description" class="form-input" placeholder="Description.." type="text">
							</div>
						</div>

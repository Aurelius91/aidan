...
class Upgrade_1_0_3_to_1_0_4 extends CI_Controller
{
	private $_setting;
	...
	public function index()
	{
		...
		try
		{
			...
			if ($this->_setting->system_version != '1.0.3')
			{
				return;
			}
	...
	{
		$this->db->simple_query("UPDATE `setting` SET `value` = '1.0.4' WHERE `name` = 'system_version';");
	}

	private function _upgrade()
	{
		$this->db->simple_query("ALTER TABLE `currency` ADD COLUMN `currency_real` DECIMAL(16,3) UNSIGNED DEFAULT 0.000 NOT NULL AFTER `currency_exchange`;");
		$this->db->simple_query("UPDATE `currency` SET `currency_real` = '1' WHERE `id` = '1';");
		$this->db->simple_query("UPDATE `currency` SET `currency_real` = '14626' WHERE `id` = '2';");
		$this->db->simple_query("UPDATE `currency` SET `currency_real` = '10631.27' WHERE `id` = '3';");
		$this->db->simple_query("UPDATE `currency` SET `currency_real` = '10554.40' WHERE `id` = '4';");
	}
}
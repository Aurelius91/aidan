...
class Sale extends CI_Controller
{
	...
	public function edit($sale_id = 0)
	{
		...
		if ($this->_setting->system_mac != 'webshop_2_aidan')
		{
		...
		$arr_sale_item_id = $this->cms_function->extract_records($arr_sale_item, 'id');
		...
		$arr_voucher_code_lookup = array();

		if (count($arr_sale_item_id) > 0)
		{
			$this->db->where_in('sale_item_id', $arr_sale_item_id);
			$arr_voucher_code = $this->core_model->get('voucher_code');

			foreach ($arr_voucher_code as $voucher_code)
			{
				$arr_voucher_code_lookup[$voucher_code->sale_item_id][] = clone $voucher_code;
			}
		}
		...
		$arr_data['arr_product'] = $this->_get_product();
		$arr_data['arr_statement'] = $this->_get_statement();
		$arr_data['arr_voucher_code_lookup'] = $arr_voucher_code_lookup;
	...
	public function ajax_edit($sale_id)
	{
		...
		try
		{
            ...
            {
            	// check status
            	// kalo belum di shipped ga boleh complete
            	if (($old_sale_record['status'] == 'pending' || $old_sale_record['status'] == 'capture' || $old_sale_record['status'] == 'settlement') && $sale_record['status'] == 'complete')
            	{
            		throw new Exception('Please make sure all item has already shipped!');
            	}

            	// kalo uda di shipped ga boleh balik ke pending
            	if ($old_sale_record['status'] == 'shipped' && ($sale_record['status'] == 'pending' || $sale_record['status'] == 'capture' || $sale_record['status'] == 'settlement'))
            	{
            		throw new Exception('This Sale has already been shipped!');
            	}

            	// kalo uda complete ga bisa di apa2in
            	if ($old_sale_record['status'] == 'complete' && $sale_record['status'] != 'complete')
            	{
            		throw new Exception('This Sale has been completed!');
            	}

            	// update stock
            	if ($sale_record['status'] == 'shipping')
            	{
            		$this->db->where('sale_id', $sale_id);
            		$arr_sale_item = $this->core_model->get('sale_item');
            		...
            		foreach ($arr_sale_item as $sale_item)
            		{
            			if ($sale_item->product_id > 0)
            			{
							...
							$stock_record['location_type'] = $sale_item->location_type;
							$stock_record['location_number'] = $sale_item->location_number;
							$stock_record['location_name'] = $sale_item->location_name;
							$stock_record['location_date'] = $sale_item->location_date;
							$stock_record['location_status'] = $sale_item->location_status;

							$stock_record['product_type'] = $sale_item->product_type;
							$stock_record['product_number'] = $sale_item->product_number;
							$stock_record['product_name'] = $sale_item->product_name;
							$stock_record['product_date'] = $sale_item->product_date;
							$stock_record['product_status'] = $sale_item->product_status;

							$stock_record['ref_type'] = $sale_item->sale_type;
							$stock_record['ref_number'] = $sale_item->sale_number;
							$stock_record['ref_name'] = $sale_item->sale_name;
							$stock_record['ref_date'] = $sale_item->sale_date;
							$stock_record['ref_status'] = $sale_item->sale_status;
